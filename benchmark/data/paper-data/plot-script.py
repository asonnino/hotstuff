...
class LogAggregator:
    def __init__(self, system, files, max_latencies, end_to_end=True):
        ...
        for chunk in data.replace(',', '').split('SUMMARY')[1:]:
            if chunk:
                records[Setup.from_str(
                    chunk)] += [Result.from_str(chunk, end_to_end)]
...
if __name__ == '__main__':
    ...
    all_systems = ['baseline', 'sqrt', 'sqrt-no-payload']

    # Parse the results.
    for system in all_systems:
        [os.remove(x) for x in glob(f'{system}.*.txt')]
        files = glob(join(system, '*.txt'))
        if 'no-payload' in system:
            end_to_end = False
        else:
            end_to_end = True
        LogAggregator(system, files, max_latencies, end_to_end).print()

    # Plot 'Happy path' graph.
    ploter = Ploter(width=6.4)
    for system in all_systems:
        ploter.plot_latency(system, [0], [10, 20, 30], [1], 512)
    ploter.finalize(
        'common-case',
        legend_cols=1,
        top_lim=25_000,
