# This script produces the plot used in the paper [](). Its
...
class LogAggregator:
    def __init__(self, files, max_latencies):
...
class Plotter:
    def __init__(self, results):
        if not results or results is None:
            PlotError('No data to plot')

        self.results = results

    def _nodes(data):
        return int(search(r'Committee size: (\d+)', data).group(1))

    def _variable(self, data):
        return [int(x) for x in findall(r'Variable value: X=(\d+)', data)]

    def _latency(self, data, scale=1_000):
    ...
    def _plot(self, title, y_axis, x_label, y_label):
        plt.figure()
        y_values, y_err = y_axis(self.results)
        x_values = self._variable(self.results)
        ...
        for x in ['pdf', 'png']:
            plt.savefig(f'{title}.{x}', bbox_inches='tight')

    def plot_latency(self):
        x_label = 'Number of Replicas'
        self._plot('latency', self._latency, x_label, 'Latency (s)')

    def plot_tps(self):
        x_label = 'Number of Replicas'
        self._plot('tps', self._tps, x_label, 'Throughput (tx/s)')
