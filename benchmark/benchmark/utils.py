...
class PathMaker:
    ...
    @staticmethod
    def result_file(faults, nodes, rate, tx_size, latency, bandwidth, clients, topology):
        return join(
            PathMaker.results_path(), 
            f'bench-{topology}-{faults}-{nodes}-{clients}-{rate}-{tx_size}-{latency}-{bandwidth}.txt'
        )
    ...
    @staticmethod
    def agg_file(type, topology, faults, nodes, clients, rate, tx_size, tc_latency, tc_bandwidth, max_latency):
        return join(
            PathMaker.plots_path(),
            f'{type}-{topology}-{faults}-{nodes}-{clients}-{rate}-{tx_size}-{tc_latency}-{tc_bandwidth}-{max_latency}.txt'
        )
